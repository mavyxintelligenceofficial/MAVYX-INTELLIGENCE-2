...
/**
 * Global AI Analysis Store — PERSISTED to localStorage
...
export const useAIStore = create<AIState>()(
  persist(
    (set, get) => ({
      ...
      setResult: (result) => {
        ...
        set(state => ({
          ...
          journal: [entry, ...state.journal].slice(0, 100), // Keep last 100
          chatMessages: [
            ...state.chatMessages,
            { role: 'assistant' as const, text: `Analysis complete for ${result.symbol}: ${result.recommendation?.toUpperCase()} with ${result.confidence}% confidence. ${result.key_evidence?.[0] || ''}` }
          ],
        }));
      },
      ...
      addToJournal: (entry) => set(state => ({
        journal: [entry, ...state.journal].slice(0, 100),
      })),
    ...
    {
      name: 'mavyx-ai-store', // localStorage key
      partialize: (state) => ({
        // Only persist these fields
        journal: state.journal,
        totalAnalyses: state.totalAnalyses,
        recommendationCounts: state.recommendationCounts,
